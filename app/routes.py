    ...
    if current_user.role in ['teacher', 'school_admin', 'system_admin']:
        ...
        for student in students:
            ...
            students_stats.append({
                ...
                'answered_questions': student_stats.get('answered_questions', 0),
...
def import_questions():
    ...
    if request.method == 'POST':
        ...
        if file.filename.endswith('.json'):
            imported, skipped, msg = import_from_json(filepath, current_user.id)
        else:
            delimiter = request.form.get('delimiter', ';')
            imported, skipped, msg = import_from_csv(filepath, current_user.id, delimiter)
...
def export_questions(format):
    """Експорт питань"""
    if format == 'json':
        filepath = 'export/questions.json'
        count = export_to_json(filepath)
        flash(f'Експортовано {count} питань в JSON', 'success')
    elif format == 'csv':
        filepath = 'export/questions.csv'
        count = export_to_csv(filepath)
        flash(f'Експортовано {count} питань в CSV', 'success')
    else:
        flash('Невідомий формат', 'danger')
...
@main_bp.route('/admin/user/edit', methods=['POST'])
@login_required
@admin_required
def admin_edit_user():
    ...
    # Для системного адміністратора - дозволяємо змінювати школу
    if current_user.role == 'system_admin':
        user.school_id = request.form.get('school_id') or None
    
    user.is_active = 'is_active' in request.form
    
    # Оновлюємо термін дії
    expiry_type = request.form.get('expiry_type', 'unlimited')
    if expiry_type == 'days':
        days = int(request.form.get('expiry_days', 30))
        user.activate_for_days(days)
    ...
    flash(f'Користувача {user.username} оновлено', 'success')
    return redirect(url_for('main.admin'))
...
def export_users(format):
    ...
    if format == 'json':
        filepath = 'export/users.json'
        count = export_users_to_json(filepath)
        flash(f'Експортовано {count} користувачів в JSON', 'success')
    elif format == 'csv':
        filepath = 'export/users.csv'
        ...
        flash(f'Експортовано {len(users)} користувачів в CSV', 'success')
...
def group_detail(group_id):
    ...
    return render_template('group_detail.html', group=group, categories=categories, questions=questions)
